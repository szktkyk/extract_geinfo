...
def main():
    # 処理するPMIDのリストを読み込む
    pmids_list = []
    with open(config.PATH["pmids_results"], "r") as f:
        for line in f:
    ...
    # 文献情報を読み込む
    df_pub = pl.read_csv(f"./ref_data/20240510_pubdetails.csv")
    print(df_pub.head(5))

    # LLMの呼び出し
    chat = ChatOpenAI(openai_api_key=os.getenv("openai_apikey"),temperature=0, model="gpt-4")
    # parserの呼び出し
    ...
    with get_openai_callback() as cb:
        for pmid in pmids_list:
            ...
            with open(f'./logs/{config.date}_llm_log.txt', 'a') as f:
